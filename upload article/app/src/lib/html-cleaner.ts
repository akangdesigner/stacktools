...
export function cleanHtml(rawHtml: string, client: ClientProfile): string {
  ...
  // ── 9. Append CTA links at the end of the article
  ...
  if (validCtaLinks.length > 0) {
    const linksHtml = validCtaLinks
      .map(
        (c) =>
          `<a href="${c.url}" style="color: ${client.linkColor}; text-decoration: ${client.linkTextDecoration}; font-weight: ${client.linkFontWeight};">${c.text}</a>`
      )
      .join("　｜　");
