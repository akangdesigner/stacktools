...
- [x] （2026-08-17）**發現抓取失敗的品牌不會提早結束，還是會白跑「官方資訊」＋「轉化html卡片」兩次 LLM 呼叫**，換來的只是一張佔位卡片，白燒錢。`推薦文-3-完整生成` 新增兩層 IF 判斷（`fetch_status`）：抓取失敗直接組固定的佔位資料，完全跳過這兩個 LLM 節點。連帶修正一個潛在舊 bug：`Merge2` 原本用「位置比對」品牌規格資料跟品牌深度研究資料，只要有品牌抓取失敗導致順序跑掉，就會把 A 品牌的規格接到 B 品牌卡片上——改成用 `brand_name` 欄位精準比對（兩邊都同源自「拆分品牌」，字串一定一致，不用模糊比對）。n8n_validate_workflow 0 error；**這次重構範圍較大（新增6個節點），只做過結構驗證，沒有實測過「其中一品牌抓取失敗」的真實案例**（要驗證得花錢跑完整流程，故意沒跑）
