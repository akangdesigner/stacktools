...
- [ ] 待辦（2026-08-18 記）：
  1. 確認卡片樣式——上面的新版卡片設計/引用規則都還沒花錢跑過完整真實生成測試，要找時間實際測一輪確認線上效果
  2. 作者區塊——目前品牌卡片跟文章都沒有「作者」欄位/區塊，小積木要加，細節待討論
  3. 推薦文內容重點化——現有內文（前言/前三章/總結/FAQ）都是整段文字，沒有特別標出重點，要規劃怎麼把關鍵內容視覺化凸顯（例如加粗/條列/重點色字），細節待討論
