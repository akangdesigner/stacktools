...
- [ ] **卡片內容還是空泛（官方規格+UX評價都是「目前無公開資料」/空陣列，「小編點評」是 AI 沒資料時的通用罐頭文字）**，跟上面兩個已修的 bug 是不同層次問題：品牌清單的 `official_url` 從工作流1查詢時就是空字串，導致官方網站抓規格那段直接 `Invalid URL` 失敗；UX 討論搜尋（Dcard/PTT）也常年查無資料。這塊是上游資料品質/搜尋來源的問題，範圍更大，還沒動手，需要另外討論怎麼處理（換搜尋詞、放寬品牌數量門檻、或部分品牌容許卡片精簡）
- 8/14 OpenRouter 花費：$5.05；8/17 兩輪全流程實測另計
