...
function getDb() {
  ...
  _db = new Database(path.join(DATA_DIR, 'blog-gen.db'));
  ...
  const existingCols = (_db.prepare("PRAGMA table_info(blog_gen_clients)").all() as { name: string }[]).map(c => c.name);
  for (const col of ['wp_url', 'wp_username', 'wp_app_password', 'wp_category_id', 'h2_color', 'h2_size', 'h3_color', 'h3_size']) {
    if (!existingCols.includes(col)) {
      _db.exec(`ALTER TABLE blog_gen_clients ADD COLUMN ${col} TEXT NOT NULL DEFAULT ''`);
    }
  }
  for (const [col, def] of [['faq_q_color', "'#000000'"], ['faq_q_size', "'16px'"]]) {
    if (!existingCols.includes(col)) {
      _db.exec(`ALTER TABLE blog_gen_clients ADD COLUMN ${col} TEXT NOT NULL DEFAULT ${def}`);
    }
  }
...
export function listClients(): BlogGenClient[] {
  return getDb().prepare('SELECT * FROM blog_gen_clients ORDER BY id').all() as BlogGenClient[];
}

export function getClient(id: number): BlogGenClient | undefined {
  return getDb().prepare('SELECT * FROM blog_gen_clients WHERE id = ?').get(id) as BlogGenClient | undefined;
}

export function getClientByJobId(jobId: string): BlogGenClient | undefined {
  return getDb().prepare('SELECT * FROM blog_gen_clients WHERE job_id = ?').get(jobId) as BlogGenClient | undefined;
}

export function createClient(name: string): BlogGenClient {
  const result = getDb().prepare('INSERT INTO blog_gen_clients (name) VALUES (?)').run(name);
...
export function updateClient(
  ...
  getDb().prepare('UPDATE blog_gen_clients SET name = ?, word_url = ?, gdrive_url = ?, persona = ?, wp_url = ?, wp_username = ?, wp_app_password = ?, wp_category_id = ?, h2_color = ?, h2_size = ?, h3_color = ?, h3_size = ?, faq_q_color = ?, faq_q_size = ? WHERE id = ?')
...
export function deleteClient(id: number): void {
  getDb().prepare('DELETE FROM blog_gen_clients WHERE id = ?').run(id);
}

export function setJobProcessing(id: number, jobId: string): void {
  getDb().prepare("UPDATE blog_gen_clients SET job_id = ?, job_status = ?, job_result = ?, job_updated = datetime('now') WHERE id = ?")
    .run(jobId, 'processing', '', id);
}

export function setJobResult(jobId: string, status: string, result: string): void {
  getDb().prepare("UPDATE blog_gen_clients SET job_status = ?, job_result = ?, job_updated = datetime('now') WHERE job_id = ?")
    .run(status, result, jobId);
}

export function resetJob(id: number): void {
  getDb().prepare("UPDATE blog_gen_clients SET job_id = '', job_status = '', job_result = '', job_updated = '' WHERE id = ?")
    .run(id);
}
