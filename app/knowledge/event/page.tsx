    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-amber-50 px-4 py-10">
      <div className="max-w-3xl mx-auto space-y-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-gray-900">🎤 講座與活動分享</h1>
            <p className="text-xs text-gray-400 mt-0.5">研討會、講座筆記與活動心得整理</p>
          </div>
        </div>
        ...
        {articles.length === 0 ? (
          ...
          <div className="flex flex-col gap-3">
            {articles.map(a => (
              <Link
                key={a.id}
                href={`/knowledge/event/${a.id}`}
                className="bg-white rounded-xl border border-gray-100 shadow-sm p-5 hover:border-amber-300 hover:shadow-md transition-all"
              >
