...
export default async function SEOArticlePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const article = db.prepare('SELECT * FROM articles WHERE id = ? AND category = ?').get(id, 'seo') as { title: string; content: string; published_at: string; source_url: string } | undefined;
  if (!article) notFound();
