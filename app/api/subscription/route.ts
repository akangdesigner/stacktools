...
export async function POST(req: NextRequest) {
  ...
  const { name, category, amount, currency, cycle, next_billing_date, status, note } = body;

  if (!name || !category || amount == null) {
    return NextResponse.json({ error: '缺少必填欄位' }, { status: 400 });
  }

  const sub = createSubscription({ name, category, amount, currency, cycle, next_billing_date, status, note });
