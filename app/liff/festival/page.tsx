...
export default function FestivalLiffPage() {
  ...
  // 改圖時 phase 會切回 image 再回 ready，也靠這個依賴在回來時重新撐高。
  useEffect(() => {
  ...
  async function runAll(lineUid: string, topic: string) {
    ...
    // ② 生圖（延續同一輪計時，不重置）
    await genImage(tData.imagePrompt || '', '', false, lineUid);
  }
        ...
        <section className="card">
          <div className="card-pad loading-card">
            ...
            <div className="prog-meta">
              <span className="st">{PHASE_LABEL[phase]}<span className="dots"><i>.</i><i>.</i><i>.</i></span></span>
...
.fp .pv-body-input {
  ...
  font-size: 16px; line-height: 1.75; color: #46506A;
...
.fp .field input { flex: 1; min-width: 0; border: 0; background: transparent; outline: none; font-family: var(--sans); font-size: 16px; color: var(--ink); }
