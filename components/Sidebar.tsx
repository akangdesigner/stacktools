...
type NavItem = { href: string; label: string; inDev?: boolean; suspended?: boolean; hrOnly?: boolean; icon: React.ReactNode };
...
const extraItems: { href: string; label: string; inDev?: boolean; suspended?: boolean; external?: boolean; icon: React.ReactNode }[] = [
  ...
  {
    href: 'https://productmonitor.zeabur.app',
    label: '美妝競品監控台',
    external: true,
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
  ...
  return (
    ...
    <aside className="w-56 shrink-0 border-r border-gray-200 bg-white flex flex-col min-h-screen">
      ...
      <nav className="flex-1 py-4 px-2 space-y-4 overflow-y-auto">
        ...
        {/* 行銷部 */}
        <div>
          <p className="px-3 mb-1 text-xs font-semibold text-gray-400 uppercase tracking-wider">行銷部</p>
        ...
        {/* HR 總部 */}
        <div>
          <p className="px-3 mb-1 text-xs font-semibold text-gray-400 uppercase tracking-wider">HR 總部</p>
          <div className="space-y-1">
            {hrItems.map((item) => {
              const isActive = pathname === item.href || pathname.startsWith(item.href + '/');
              return (
        ...
        {techItems.length > 0 && (
          <div>
            <p className="px-3 mb-1 text-xs font-semibold text-gray-400 uppercase tracking-wider">技術部</p>
            <div className="space-y-1">
              {techItems.map((item) => {
                const isActive = pathname === item.href || pathname.startsWith(item.href + '/');
                return (
        ...
        <div>
          ...
          <div className="space-y-1">
            {extraItems.map((item) => {
              const isActive = pathname === item.href || pathname.startsWith(item.href + '/');
              const cls = `flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-colors ${isActive ? 'bg-gray-900 text-white' : 'text-gray-600 hover:bg-gray-100 hover:text-gray-900'}`;
              const inner = (
                <>
                  {item.icon}
                  <span className="flex-1">{item.label}</span>
                  {item.suspended && <span className="text-xs text-gray-400 font-normal">暫時下架</span>}
                  {item.inDev && !isActive && <span className="text-xs text-amber-500 font-normal">開發中</span>}
                </>
              );
              if (item.suspended) {
                return <button key={item.href} onClick={() => setShowSuspended(true)} className={cls + ' w-full text-left'}>{inner}</button>;
              }
              if (item.external) {
                return <a key={item.href} href={item.href} target="_blank" rel="noopener noreferrer" className={cls}>{inner}</a>;
              }
